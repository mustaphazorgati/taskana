...
| `423 Locked`
| A different process is currently blocking the requested resource and thus it cannot be modified.
|===

== Common Fields

Taskana uses Spring HATEOAS to achieve the best possible REST-conformity. +
In HATEOAS every entity contains an object *_links*. Links for navigation are found within that object.
...
= include::{snippets}/task-comment-controller-rest-documentation/get-specific-task-comment-doc-test/http-request.adoc[]
...
include::{snippets}/GetAllClassificationsDocTest/request-parameters.adoc[]
